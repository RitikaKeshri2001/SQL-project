...
-- country, worlwide_gross_income, languages and production_company contains null values.
-- Type your code below:
SELECT 
...
FROM movie GROUP BY month ORDER BY month;
...
select genre.genre, count(movie.title) as num_of_movies from movie 
inner join genre on genre.movie_id = movie.id group by genre.genre order by count(movie.title) desc;
...
SELECT COUNT(*) AS movies_with_single_genre FROM (SELECT m.id FROM movie m LEFT JOIN 
genre g ON m.id = g.movie_id GROUP BY m.id HAVING COUNT(g.movie_id) = 1) AS single_genre_movies;
...
select genre.genre, avg(movie.duration) as avg_duration from genre join 
movie on movie.id = genre.movie_id group by genre.genre order by avg_duration;
...
FROM genre AS g INNER JOIN movie as m on g.movie_id = m.id INNER JOIN ratings as r on m.id = r.movie_id
WHERE m.year = 2017 AND country = 'USA' AND r.total_votes>1000 GROUP BY g.genre ORDER BY movie_count desc;
...
SELECT m.title, r.avg_rating, g.genre FROM movie AS m INNER JOIN ratings AS r
ON m.id = r.movie_id INNER JOIN genre AS g ON m.id = g.movie_id
...
SELECT COUNT(m.id) AS movies FROM movie AS m INNER JOIN ratings AS r ON m.id = r.movie_id
...
SELECT 
	CASE WHEN 
        SUM(CASE WHEN m.languages LIKE '%German%' THEN r.total_votes ELSE 0 END) > 
        SUM(CASE WHEN m.languages LIKE '%Italian%' THEN r.total_votes ELSE 0 END) 
    THEN 'Yes' ELSE 'No' END AS German_Movie_Is_Popular_Than_Italian_Movie 
FROM movie AS m INNER JOIN ratings AS r ON m.id = r.movie_id;
...
WITH genre_top3 AS (
    ...
    WHERE r.avg_rating > 8 GROUP BY genre ORDER BY movie_count DESC LIMIT 3
)
...
GROUP BY director_name ORDER BY movie_count DESC
LIMIT 3;
...
FROM ratings AS r INNER JOIN role_mapping AS rm ON rm.movie_id = r.movie_id 
INNER JOIN names AS n ON rm.name_id = n.id
WHERE r.median_rating >= 8 AND rm.category = 'actor' GROUP BY actor_name
...
WHERE category='actor' AND country= 'India' GROUP BY name HAVING COUNT(m.id)>=5 LIMIT 3;
...
INNER JOIN role_mapping AS rm ON m.id=rm.movie_id 
INNER JOIN names AS nm ON rm.name_id=nm.id
WHERE rm.category='actress' AND m.country LIKE '%India%' AND m.languages LIKE '%Hindi%' GROUP BY name
HAVING COUNT(m.country='India')>=3 LIMIT 5;
...
FROM movie AS m INNER JOIN ratings AS r ON m.id = r.movie_id 
INNER JOIN genre AS g ON m.id = g.movie_id WHERE g.genre = 'Thriller' ORDER BY r.avg_rating DESC;
...
WHERE category='actress' AND avg_rating>8 AND g.genre='Drama' GROUP BY name LIMIT 3;
...
SELECT 
	...
	DATEDIFF(LEAD(date_published) OVER (PARTITION BY n.id ORDER BY m.date_published),date_published)
FROM 
	names AS n
		INNER JOIN
	director_mapping AS d
...
FROM movie_summary GROUP BY director_id
